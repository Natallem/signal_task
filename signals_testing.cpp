...
TEST(signal_testing, disconnect_in_emit)
{
    ...
    conn2.reset(new connection(sig.connect([&] { ++got2; conn2.reset(); })));
    uint32_t got3 = 0;
...
TEST(signal_testing, destroy_signal_in_emit)
{
    ...
    uint32_t got2 = 0;
    connection conn2(sig->connect([&] { ++got2; sig.reset(); }));
    uint32_t got3 = 0;
...
TEST(signal_testing, move_in_emit_02)
{
    ...
    conn1_old = std::make_unique<connection>(sig.connect([&]
    {
        ++got1;
    }));
    ...
    conn3_old = std::make_unique<connection>(sig.connect([&]
    {
        ++got3;
    }));
